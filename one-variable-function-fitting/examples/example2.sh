#!/bin/bash

FX="np.sin(x)"
python ../fx_gen.py --dsout datasets/example2_train.csv --fx "$FX" --rbegin -6.0 --rend 6.0 --rstep 0.01
python ../fx_fit.py --trainds datasets/example2_train.csv --modelout models/example2 \
  --epochs 750 --batch_size 100 --learning_rate 0.005 \
  --optimizer Adam --epsilon 1e-7 \

python ../fx_gen.py --dsout datasets/example2_test.csv  --fx "$FX" --rbegin -6.0 --rend 6.0 --rstep 0.0475
python ../fx_predict.py --model models/example2 --testds datasets/example2_test.csv --predictedout predictions/example2_pred.csv

python ../fx_plot.py --trainds datasets/example2_train.csv --predicted predictions/example2_pred.csv
#python ../fx_plot.py --trainds datasets/example2_train.csv --predicted predictions/example2_pred.csv --savefig predictions/example2.png
